	...
	unsafe partial class StbVorbis
	{
		...
		public static int start_decoder(stb_vorbis f)
		{
			...
			if (vorbis_validate(header) == 0)
				return error(f, STBVorbisError.VORBIS_invalid_setup);
			len = get32_packet(f);
			f.vendor = (sbyte*) setup_malloc(sizeof(char) * (len + 1));
			if (f.vendor == null)
				return error(f, STBVorbisError.VORBIS_outofmem);
			for (i = 0; i < len; ++i) f.vendor[i] = (sbyte) get8_packet(f);
			...
			{
				f.comment_list = (sbyte**) setup_malloc(sizeof(char*) * f.comment_list_length);
			...
			{
				len = get32_packet(f);
				f.comment_list[i] = (sbyte*) setup_malloc(sizeof(char) * (len + 1));
				if (f.comment_list[i] == null)
					return error(f, STBVorbisError.VORBIS_outofmem);
				for (j = 0; j < len; ++j) f.comment_list[i][j] = (sbyte) get8_packet(f);
		...
		public static void vorbis_deinit(stb_vorbis p)
		{
			...
			for (i = 0; i < p.comment_list_length; ++i) CRuntime.free(p.comment_list[i]);
