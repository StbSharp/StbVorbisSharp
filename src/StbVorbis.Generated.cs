...
namespace StbVorbisSharp
{
	unsafe partial class StbVorbis
	{
		...
		public unsafe struct MappingChannel
		{
		...
		public unsafe struct ProbedPage
		{
		...
		public unsafe struct stb_vorbis_info
		{
		...
		public unsafe struct stbv__floor_ordering
		{
			public ushort x;
			public ushort id;
		}
		...
		{
			int i = 0; int z = (int)(codebook_decode_start(f, c));
			if ((z) < (0))
				return (int)(0);
			if ((len) > (c->dimensions))
		...
		public static int codebook_decode_deinterleave_repeat(stb_vorbis f, Codebook* c, float** outputs, int ch, int* c_inter_p, int* p_inter_p, int len, int total_decode)
		{
			...
			int i = 0; int z = 0; int effective = (int)(c->dimensions);
			if ((c->lookup_type) == (0))
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_stream)));
		...
		{
			int i = 0; int k = 0; int m = (int)(0);
			uint* available = stackalloc uint[32];
			CRuntime.memset(available, (int)(0), (ulong)(32 * sizeof(uint)));
			for (k = (int)(0); (k) < (n); ++k)
			{
				if ((len[k]) < (255))
					break;
		...
		public static void compute_samples(int mask, short* output, int num_c, float*[] data, int d_offset, int len)
		{
			...
			int i = 0; int j = 0; int o = 0; int n = (int)(32);
			for (o = (int)(0); (o) < (len); o += (int)(32))
			{
				CRuntime.memset(buffer, (int)(0), (ulong)(32 * sizeof(float)));
				if ((o + n) > (len))
					n = (int)(len - o);
		...
		{
			float* buffer = stackalloc float[32];
			int i = 0; int j = 0; int o = 0; int n = (int)(32 >> 1);
		...
		public static void convert_samples_short(int buf_c, short** buffer, int b_offset, int data_c, float*[] data, int d_offset, int samples)
		{
			...
			if (((buf_c != data_c) && ((buf_c) <= (2))) && ((data_c) <= (6)))
			{
				for (i = (int)(0); (i) < (buf_c); ++i)
				{
					compute_samples((int)(convert_samples_short_channel_selector[buf_c, i]), buffer[i] + b_offset, (int)(data_c), data, (int)(d_offset), (int)(samples));
				}
			}
			else
			{
		...
		public static void decode_residue(stb_vorbis f, float** residue_buffers, int ch, int n, int rn, byte* do_not_decode)
		{
			...
			ArrayBuffer2D<IntPtr> part_classdata = f.PtrBuffer2D;
					...
					if ((ch) == (2))
					{
						while ((pcount) < (part_read))
						{
							...
							if ((pass) == (0))
							{
								...
								int q = 0;
								q = (int)(codebook_decode_scalar(f, codebook));
								if ((codebook->sparse) != 0)
									q = (int)(codebook->sorted_values[q]);
								if ((q) == (-1))
									goto done;
							...
							for (i = (int)(0); ((i) < (classwords)) && ((pcount) < (part_read)); ++i, ++pcount)
							{
								...
								int c2 = (int)(part_classdata[0, class_set].ToBytePointer()[i]);
								int b = (int)(r.residue_books[c2, pass]);
								if ((b) >= (0))
								{
									Codebook* book = f.codebooks + b;
									if (codebook_decode_deinterleave_repeat(f, book, residue_buffers, (int)(ch), &c_inter, &p_inter, (int)(n), (int)(r.part_size)) == 0)
										goto done;
								}
								else
								{
				...
				while ((pcount) < (part_read))
				{
					if ((pass) == (0))
					{
						for (j = (int)(0); (j) < (ch); ++j)
						{
							if (do_not_decode[j] == 0)
							{
								Codebook* codebook = f.codebooks + r.classbook;
								int temp = 0;
								temp = (int)(codebook_decode_scalar(f, codebook));
								if ((codebook->sparse) != 0)
									temp = (int)(codebook->sorted_values[temp]);
								if ((temp) == (-1))
									goto done;
					...
					for (i = (int)(0); ((i) < (classwords)) && ((pcount) < (part_read)); ++i, ++pcount)
					{
						for (j = (int)(0); (j) < (ch); ++j)
						{
							if (do_not_decode[j] == 0)
							{
								int c2 = (int)(part_classdata[j, class_set].ToBytePointer()[i]);
								int b = (int)(r.residue_books[c2, pass]);
								if ((b) >= (0))
								{
		...
		{
			int n2 = (int)(n >> 1);
			int s = (int)(map->chan[i].mux); int floor = 0;
		...
		public static void draw_line(float* output, int x0, int y0, int x1, int y1, int n)
		{
			...
			if ((x) < (x1))
			{
				output[x] *= (float)(inverse_db_table[y & 255]);
				for (++x; (x) < (x1); ++x)
				{
					err += (int)(ady);
					if ((err) >= (adx))
		...
		public static float float32_unpack(uint x)
		{
			...
			return (float)(CRuntime.ldexp((double)((float)(res)), (int)((int)(exp) - 788)));
		}
		...
		{
			byte* header = stackalloc byte[27]; byte* lacing = stackalloc byte[255];
			int i = 0; int len = 0;
		...
		{
			uint previous_safe = 0; uint end = 0;
		...
		public static int ilog(int n)
		{
			...
			if ((n) < (1 << 14))
				if ((n) < (1 << 4))
					return (int)(0 + ilog_log2_4[n]);
				else if ((n) < (1 << 9))
					return (int)(5 + ilog_log2_4[n >> 5]);
				else
					return (int)(10 + ilog_log2_4[n >> 10]);
			else if ((n) < (1 << 24))
				if ((n) < (1 << 19))
					return (int)(15 + ilog_log2_4[n >> 15]);
				else
					return (int)(20 + ilog_log2_4[n >> 20]);
			else if ((n) < (1 << 29))
				return (int)(25 + ilog_log2_4[n >> 25]);
			else
				return (int)(30 + ilog_log2_4[n >> 30]);
		}
		public static void imdct_step3_inner_r_loop(int lim, float* e, int d0, int k_off, float* A, int k1)
		{
			int i = 0;
			float k00_20 = 0; float k01_21 = 0;
			float* e0 = e + d0;
			float* e2 = e0 + k_off;
		...
		public static void imdct_step3_inner_s_loop_ld654(int n, float* e, int i_off, float* A, int base_n)
		{
			...
			while ((z) > (_base_))
			{
				...
				z[-8] = (float)(k00);
				z[-9] = (float)(k11);
				z[-10] = (float)((l00 + l11) * A2);
				z[-11] = (float)((l11 - l00) * A2);
				k00 = (float)(z[-4] - z[-12]);
				k11 = (float)(z[-5] - z[-13]);
				l00 = (float)(z[-6] - z[-14]);
				l11 = (float)(z[-7] - z[-15]);
				...
				z[-12] = (float)(k11);
				z[-13] = (float)(-k00);
				z[-14] = (float)((l11 - l00) * A2);
				z[-15] = (float)((l00 + l11) * -A2);
				iter_54(z);
				iter_54(z - 8);
				z -= 16;
			}
		...
		{
			int n2 = (int)(n >> 1); int n4 = (int)(n >> 2); int n8 = (int)(n >> 3);
			f.A[b] = (float*)(setup_malloc((int)(sizeof(float) * n2)));
			f.B[b] = (float*)(setup_malloc((int)(sizeof(float) * n2)));
			f.C[b] = (float*)(setup_malloc((int)(sizeof(float) * n4)));
			if (((f.A[b] == null) || (f.B[b] == null)) || (f.C[b] == null))
				return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
			compute_twiddle_factors((int)(n), f.A[b], f.B[b], f.C[b]);
			f.window[b] = (float*)(setup_malloc((int)(sizeof(float) * n2)));
			if (f.window[b] == null)
				return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
			compute_window((int)(n), f.window[b]);
			f.bit_reverse[b] = (ushort*)(setup_malloc((int)(sizeof(ushort) * n8)));
		...
		{
			int n2 = (int)(n >> 1); int n4 = (int)(n >> 2); int n8 = (int)(n >> 3); int l = 0;
			int ld = 0;
			int save_point = (int)((f).temp_offset);
			float* u = null; float* v = null;
		...
		{
			int s = (int)(f.next_seg); int first = (int)(1);
			byte* p = f.stream;
			if (s != -1)
			{
				...
				if ((s) == (f.segment_count))
					s = (int)(-1);
				if ((p) > (f.stream_end))
					return (int)(error(f, (STBVorbisError.VORBIS_need_more_data)));
				first = (int)(0);
			}
		...
		{
			float k00 = 0; float k11 = 0; float k22 = 0; float k33 = 0;
		...
		public static int lookup1_values(int entries, int dim)
		{
			...
			if (((int)(CRuntime.floor((double)(CRuntime.pow((double)((float)(r) + 1), (double)(dim)))))) <= (entries))
				++r;
			if ((CRuntime.pow((double)((float)(r) + 1), (double)(dim))) <= (entries))
				return (int)(-1);
			if (((int)(CRuntime.floor((double)(CRuntime.pow((double)((float)(r)), (double)(dim)))))) > (entries))
				return (int)(-1);
			return (int)(r);
		}
		...
		public static int maybe_start_packet(stb_vorbis f)
		{
			if ((f.next_seg) == (-1))
			{
				...
				if ((f.eof) != 0)
					return (int)(0);
				if (0x4f != x)
					return (int)(error(f, (STBVorbisError.VORBIS_missing_capture_pattern)));
				if (0x67 != get8(f))
					return (int)(error(f, (STBVorbisError.VORBIS_missing_capture_pattern)));
				if (0x67 != get8(f))
					return (int)(error(f, (STBVorbisError.VORBIS_missing_capture_pattern)));
				...
				if ((f.page_flag & 1) != 0)
				{
					...
					return (int)(error(f, (STBVorbisError.VORBIS_continued_packet_flag_invalid)));
				}
			}
		...
		{
			int bits_read = 0; int bytes_read = 0;
		...
		{
			ProbedPage left = new ProbedPage(); ProbedPage right = new ProbedPage(); ProbedPage mid = new ProbedPage();
			int i = 0; int start_seg_with_known_loc = 0; int end_pos = 0; int page_start = 0;
			uint delta = 0; uint stream_length = 0; uint padding = 0; uint last_sample_limit = 0;
			double offset = (double)(0.0); double bytes_per_sample = (double)(0.0);
			int probe = (int)(0);
			stream_length = (uint)(stb_vorbis_stream_length_in_samples(f));
			if ((stream_length) == (0))
				return (int)(error(f, (STBVorbisError.VORBIS_seek_without_length)));
			if ((sample_number) > (stream_length))
				return (int)(error(f, (STBVorbisError.VORBIS_seek_invalid)));
			padding = (uint)((f.blocksize_1 - f.blocksize_0) >> 2);
			if ((sample_number) < (padding))
				last_sample_limit = (uint)(0);
			else
				last_sample_limit = (uint)(sample_number - padding);
			...
			if ((last_sample_limit) <= (left.last_decoded_sample))
			{
				if ((stb_vorbis_seek_start(f)) != 0)
				{
					if ((f.current_loc) > (sample_number))
						return (int)(error(f, (STBVorbisError.VORBIS_seek_failed)));
					return (int)(1);
				}
				...
				else
				{
					if ((probe) < (2))
					{
						if ((probe) == (0))
						{
							...
							offset = (double)(left.page_start + bytes_per_sample * (last_sample_limit - left.last_decoded_sample));
						}
						else
						{
							double error = (double)(((double)(last_sample_limit) - mid.last_decoded_sample) * bytes_per_sample);
		...
		{
			byte* header = stackalloc byte[6]; byte x = 0; byte y = 0;
			int len = 0; int i = 0; int j = 0; int k = 0; int max_submaps = (int)(0);
			int longest_floorlist = (int)(0);
			f.first_decode = (byte)(1);
			if (start_page(f) == 0)
				return (int)(0);
			if ((f.page_flag & 2) == 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_first_page)));
			if ((f.page_flag & 4) != 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_first_page)));
			if ((f.page_flag & 1) != 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_first_page)));
			...
			if (f.segments[0] != 30)
			{
				if (((((((((((f.segments[0]) == (64)) && ((getn(f, header, (int)(6))) != 0)) && ((header[0]) == (102))) && ((header[1]) == (105))) && ((header[2]) == (115))) && ((header[3]) == (104))) && ((header[4]) == (101))) && ((header[5]) == (97))) && ((get8(f)) == (100))) && ((get8(f)) == (0)))
					return (int)(error(f, (STBVorbisError.VORBIS_ogg_skeleton_not_supported)));
				else
					return (int)(error(f, (STBVorbisError.VORBIS_invalid_first_page)));
			}
			...
			if (getn(f, header, (int)(6)) == 0)
				return (int)(error(f, (STBVorbisError.VORBIS_unexpected_eof)));
			...
			if (f.channels == 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_first_page)));
			if ((f.channels) > (16))
				return (int)(error(f, (STBVorbisError.VORBIS_too_many_channels)));
			...
			x = (byte)(get8(f));
			{
				...
				if (((log0) < (6)) || ((log0) > (13)))
					return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
				if (((log1) < (6)) || ((log1) > (13)))
					return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
				if ((log0) > (log1))
					return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
			...
			if (vorbis_validate(header) == 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
			len = (int)(get32_packet(f));
			f.vendor = (sbyte*)(setup_malloc((int)(sizeof(char) * (len + 1))));
			if ((f.vendor) == (null))
				return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
			for (i = (int)(0); (i) < (len); ++i)
			{
				f.vendor[i] = (sbyte)(get8_packet(f));
			}

			f.vendor[len] = ((sbyte)(0));
			f.comment_list_length = (int)(get32_packet(f));
			f.comment_list = null;
			if ((f.comment_list_length) > (0))
			{
				f.comment_list = (sbyte**)(setup_malloc((int)(sizeof(char*) * (f.comment_list_length))));
				if ((f.comment_list) == (null))
					return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
			}

			for (i = (int)(0); (i) < (f.comment_list_length); ++i)
			{
				len = (int)(get32_packet(f));
				f.comment_list[i] = (sbyte*)(setup_malloc((int)(sizeof(char) * (len + 1))));
				if ((f.comment_list[i]) == (null))
					return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
				for (j = (int)(0); (j) < (len); ++j)
				{
					f.comment_list[i][j] = (sbyte)(get8_packet(f));
				}

				f.comment_list[i][len] = ((sbyte)(0));
			}

			x = (byte)(get8_packet(f));
			if ((x & 1) == 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
			skip(f, (int)(f.bytes_in_seg));
			f.bytes_in_seg = (byte)(0);
			do
			{
			...
			f.codebooks = (Codebook*)(setup_malloc((int)(sizeof(Codebook) * f.codebook_count)));
			if ((f.codebooks) == (null))
				return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
			...
			for (i = (int)(0); (i) < (f.codebook_count); ++i)
			{
				...
				if (((c->dimensions) == (0)) && (c->entries != 0))
					return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
				if ((c->sparse) != 0)
					lengths = (byte*)(setup_temp_malloc((int)(c->entries)));
				else
					lengths = c->codeword_lengths = (byte*)(setup_malloc((int)(c->entries)));
				if (lengths == null)
					return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
				if ((ordered) != 0)
				{
				...
				{
					c->codeword_lengths = (byte*)(setup_malloc((int)(c->entries)));
				...
				if (c->sparse == 0)
				{
					c->codewords = (uint*)(setup_malloc((int)(sizeof(uint) * c->entries)));
					if (c->codewords == null)
						return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
				}
				else
				{
					...
					{
						c->codeword_lengths = (byte*)(setup_malloc((int)(c->sorted_entries)));
						if (c->codeword_lengths == null)
							return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
						c->codewords = (uint*)(setup_temp_malloc((int)(sizeof(uint) * c->sorted_entries)));
						if (c->codewords == null)
							return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
						values = (uint*)(setup_temp_malloc((int)(sizeof(uint) * c->sorted_entries)));
						if (values == null)
							return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
					}
				...
				if ((c->sorted_entries) != 0)
				{
					c->sorted_codewords = (uint*)(setup_malloc((int)(sizeof(uint) * (c->sorted_entries + 1))));
					if ((c->sorted_codewords) == (null))
						return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
					c->sorted_values = (int*)(setup_malloc((int)(sizeof(int) * (c->sorted_entries + 1))));
				...
				if ((c->lookup_type) > (0))
				{
					...
					if ((c->lookup_type) == (1))
					{
						int values2 = (int)(lookup1_values((int)(c->entries), (int)(c->dimensions)));
						if ((values2) < (0))
							return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
					...
					if ((c->lookup_type) == (1))
					{
						...
						if ((sparse) != 0)
						{
							if ((c->sorted_entries) == (0))
								goto skip;
							c->multiplicands = (float*)(setup_malloc((int)(sizeof(float) * c->sorted_entries * c->dimensions)));
						}
						else
							c->multiplicands = (float*)(setup_malloc((int)(sizeof(float) * c->entries * c->dimensions)));
						...
						for (j = (int)(0); (j) < (len2); ++j)
						{
							...
							for (k = (int)(0); (k) < (c->dimensions); ++k)
							{
								...
								float val = (float)(mults[off] * c->delta_value + c->minimum_value + last);
			...
			f.floor_config = (Floor*)(setup_malloc((int)(f.floor_count * sizeof(Floor))));
			...
			for (i = (int)(0); (i) < (f.floor_count); ++i)
			{
				...
				if ((f.floor_types[i]) == (0))
				{
					...
					return (int)(error(f, (STBVorbisError.VORBIS_feature_not_supported)));
				}
				else
				{
					stbv__floor_ordering* p = stackalloc stbv__floor_ordering[250];
					...
					for (j = (int)(0); (j) <= (max_class); ++j)
					{
						...
						{
							g->subclass_books[j * 8 + k] = (short)((short)(get_bits(f, (int)(8))) - 1);
			...
			for (i = (int)(0); (i) < (f.residue_count); ++i)
			{
				...
				r.classdata = (byte**)(setup_malloc((int)(sizeof(byte*) * f.codebooks[r.classbook].entries)));
				if (r.classdata == null)
					return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
				...
				for (j = (int)(0); (j) < (f.codebooks[r.classbook].entries); ++j)
				{
					...
					int temp = (int)(j);
					r.classdata[j] = (byte*)(setup_malloc((int)(sizeof(byte) * classwords)));
			...
			f.mapping = (Mapping*)(setup_malloc((int)(f.mapping_count * sizeof(Mapping))));
			if ((f.mapping) == (null))
				return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
			...
			for (i = (int)(0); (i) < (f.mapping_count); ++i)
			{
				...
				if (mapping_type != 0)
					return (int)(error(f, (STBVorbisError.VORBIS_invalid_setup)));
				m->chan = (MappingChannel*)(setup_malloc((int)(f.channels * sizeof(MappingChannel))));
				if ((m->chan) == (null))
					return (int)(error(f, (STBVorbisError.VORBIS_outofmem)));
				if ((get_bits(f, (int)(1))) != 0)
			...
			{
				f.channel_buffers[i] = (float*)(setup_malloc((int)(sizeof(float) * f.blocksize_1)));
				f.previous_window[i] = (float*)(setup_malloc((int)(sizeof(float) * f.blocksize_1 / 2)));
				f.finalY[i] = (short*)(setup_malloc((int)(sizeof(short) * longest_floorlist)));
			...
			f.blocksize[1] = (int)(f.blocksize_1);
			{
				...
				for (var i2 = (int)(0); (i2) < (f.residue_count); ++i2)
				{
			...
			{
				f.first_audio_page_offset = (uint)(0);
			}
		...
		{
			uint loc0 = 0; uint loc1 = 0; uint n = 0;
			if (((f.first_decode) != 0) && (((f).push_mode) == 0))
			{
				f.p_first.page_start = (uint)(stb_vorbis_get_file_offset(f) - 4);
			}

			if (0 != get8(f))
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_stream_structure_version)));
			...
			{
				if (getn(f, sptr, (int)(f.segment_count)) == 0)
					return (int)(error(f, (STBVorbisError.VORBIS_unexpected_eof)));
			}
			...
			if ((f.first_decode) != 0)
			{
				...
				f.p_first.last_decoded_sample = (uint)(loc0);
			}
		...
		public static int stb_vorbis_decode_frame_pushdata(stb_vorbis f, byte* data, int data_len, int* channels, ref float*[] output, int* samples)
		{
			...
			if (((f).push_mode) == 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_api_mixing)));
		...
		{
			int data_len = 0; int offset = 0; int total = 0; int limit = 0; int error = 0;
			short* data;
			stb_vorbis v = stb_vorbis_open_memory(mem, (int)(len), &error);
			if ((v) == (null))
				return (int)(-1);
		...
		public static stb_vorbis_comment stb_vorbis_get_comment(stb_vorbis f)
		{
			stb_vorbis_comment d = new stb_vorbis_comment();
			d.vendor = f.vendor;
			d.comment_list_length = (int)(f.comment_list_length);
			d.comment_list = f.comment_list;
			return (stb_vorbis_comment)(d);
		}
		...
		{
			int len = 0; int right = 0; int left = 0; int i = 0;
			if (((f).push_mode) != 0)
				return (int)(error(f, (STBVorbisError.VORBIS_invalid_api_mixing)));
			if (vorbis_decode_packet(f, &len, &left, &right) == 0)
			{
		...
		{
			uint restore_offset = 0; uint previous_safe = 0;
			uint end = 0; uint last_page_loc = 0;
			if (((f).push_mode) != 0)
		...
		public static stb_vorbis vorbis_alloc(stb_vorbis f) => new stb_vorbis();
		...
		{
			int i = 0; int n = 0; int prev = 0; int next = 0; int window_center = 0;
			f.channel_buffer_start = (int)(f.channel_buffer_end = (int)(0));
		retry:
			...
			{
				if (((f).push_mode) != 0)
					return (int)(error(f, (STBVorbisError.VORBIS_bad_packet_type)));
		...
		{
			int mode = 0; int left_end = 0; int right_end = 0;
			if (vorbis_decode_initial(f, p_left, &left_end, p_right, &right_end, &mode) == 0)
				return (int)(0);
			fixed (Mode* mptr = &f.mode_config[mode])
			{
				return (int)(vorbis_decode_packet_rest(f, len, mptr, (int)(*p_left), (int)(left_end), (int)(*p_right), (int)(right_end), p_left));
			}
		}
		...
		{
			Mapping* map;
			int i = 0; int j = 0; int k = 0; int n = 0; int n2 = 0;
			...
			for (i = (int)(0); (i) < (f.channels); ++i)
			{
				...
				else
				{
					...
					if ((get_bits(f, (int)(1))) != 0)
					{
						...
						int range = (int)(vorbis_decode_packet_rest_range_list[g->floor1_multiplier - 1]);
						int offset = (int)(2);
						finalY = f.finalY[i];
		...
		{
			int i = 0; int j = 0;
			CRuntime.free(p.vendor);
			for (i = (int)(0); (i) < (p.comment_list_length); ++i)
			{
				CRuntime.free(p.comment_list[i]);
			}

			CRuntime.free(p.comment_list);
			...
			if ((p.codebooks) != null)
			{
				for (i = (int)(0); (i) < (p.codebook_count); ++i)
				{
					...
					if (c->sorted_values != null)
					{
						CRuntime.free(c->sorted_values - 1);
					}
				}
			...
			if ((p.mapping) != null)
			{
				...
				{
					CRuntime.free(p.mapping[i].chan);
				}
			...
			for (; ; )
			{
				...
				if ((n) == (0x4f))
				{
					...
					if ((i) == (4))
					{
						...
						{
							header[i2] = (byte)(ogg_page_header[i2]);
						}
		...
		{
			int len = 0; int right = 0; int left = 0; int res = 0;
		...
		{
			return ((CRuntime.memcmp(data, vorbis_validate_vorbis, (ulong)(6))) == (0) ? 1 : 0);
		}
	}
}